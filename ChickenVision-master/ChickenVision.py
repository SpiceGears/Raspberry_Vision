...
def findBall(contours, image, centerX, centerY):
    ...
    if len(contours) > 0:
        ...
        if (len(biggestCargo) > 0):
			...
			#Spice distance
			#calculateDistance(34, 2.54*5, pitch):
...
def findTape(contours, image, centerX, centerY):
    ...
    if len(contours) >= 2:
        ...
        for i in range(len(biggestCnts) - 1):
            ...
            if (np.sign(tilt1) != np.sign(tilt2)):
                ...
                if [centerOfTarget, yawToTarget, biggestCnts[i][3], biggestCnts[i + 1][3]] not in targets:
                    targets.append([centerOfTarget, yawToTarget, biggestCnts[i][3], biggestCnts[i + 1][3]])
    ...
    if (len(targets) > 0):
        ...
        # networkTable.putNumber("boxX31", box[3][1])
            #networkTable.putNumber("boxY", p[1])


        #wspolrzedne i wielkosci najwiekszych obiektow

  ...
  public static void main(String... args) {
    ...
    if (cameras.size() >= 1) {
      VisionThread visionThread = new VisionThread(cameras.get(0),
              new Visionv(), pipeline -> {
                //analysisOutputTable.getEntry("Hello").setString("World");
                List<Visionv.VisionTargets> hvts = pipeline.getDetectedTargets();
                Number[] bearings = new Number[hvts.size()];
                Number[] ranges = new Number[hvts.size()];
                int i = 0;
                int imgWidth = cameras.get(0).getVideoMode().width;
                for (Visionv.VisionTargets hvt : hvts) {
                  ranges[i] = hvt.computeRangeInches(imgWidth, Visionv.CAMERA_FOV_WIDTH_DEG);
                  bearings[i] = hvt.computeBearingDegrees(imgWidth, Visionv.CAMERA_FOV_WIDTH_DEG);
                  i++;
                }
                ...
                //analysisOutputTable.getEntry("IsItWorking?").setString("yeeees");
      });
      visionThread.start();
    }
